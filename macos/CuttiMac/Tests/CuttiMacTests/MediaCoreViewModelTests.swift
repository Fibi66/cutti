...
final class MediaCoreViewModelTests: XCTestCase {
    ...
    // MARK: - allTimelineClipsAnalyzed (One-click first cut re-cut gate)

    private func makeRecordWithCopilot(
        id: UUID = UUID(),
    ...
    private func dummyCopilot() -> AICopilotSnapshot {
        AICopilotSnapshot(
            semanticTags: [],
            issues: [],
            suggestions: [],
            markers: [],
            keptRanges: [TimeRange(startSeconds: 0, endSeconds: 5)]
        )
    }

    func test_allTimelineClipsAnalyzed_emptyTimeline_isFalse() {
        // Empty timeline → re-cut question doesn't apply → false (so the
        // confirm dialog never opens on a project with nothing to cut).
        let vm = MediaCoreViewModel(playbackCore: SpyPlaybackCore())
        XCTAssertFalse(vm.allTimelineClipsAnalyzed)
    }

    func test_allTimelineClipsAnalyzed_someClipsUnanalyzed_isFalse() {
        let vm = MediaCoreViewModel(playbackCore: SpyPlaybackCore())
        let analyzedID = UUID()
        let pendingID = UUID()
        vm.records = [
            makeRecordWithCopilot(id: analyzedID, copilot: dummyCopilot()),
            makeRecordWithCopilot(id: pendingID, copilot: nil),
        ]
        vm.timelineSegments = [
            TimelineSegment(id: UUID(), sourceVideoID: analyzedID, range: TimeRange(startSeconds: 0, endSeconds: 5), text: "A", subtitles: []),
            TimelineSegment(id: UUID(), sourceVideoID: pendingID, range: TimeRange(startSeconds: 0, endSeconds: 5), text: "B", subtitles: []),
        ]
        XCTAssertFalse(vm.allTimelineClipsAnalyzed,
                       "If any timeline video lacks copilot, normal first-cut should run (no confirm)")
    }

    func test_allTimelineClipsAnalyzed_allClipsAnalyzed_isTrue() {
        let vm = MediaCoreViewModel(playbackCore: SpyPlaybackCore())
        let firstID = UUID()
        let secondID = UUID()
        vm.records = [
            makeRecordWithCopilot(id: firstID, copilot: dummyCopilot()),
            makeRecordWithCopilot(id: secondID, copilot: dummyCopilot()),
        ]
        vm.timelineSegments = [
            TimelineSegment(id: UUID(), sourceVideoID: firstID, range: TimeRange(startSeconds: 0, endSeconds: 5), text: "A", subtitles: []),
            TimelineSegment(id: UUID(), sourceVideoID: secondID, range: TimeRange(startSeconds: 0, endSeconds: 5), text: "B", subtitles: []),
        ]
        XCTAssertTrue(vm.allTimelineClipsAnalyzed,
                      "If every timeline video has copilot, the next click is a re-cut → confirm")
    }

    func test_allTimelineClipsAnalyzed_ignoresLibraryOnlyRecords() {
        // Regression: pre-refactor `canStartAnalysis` looked at the
        // library; the new behavior must ignore library clips that
        // were never dragged onto the timeline.
        let vm = MediaCoreViewModel(playbackCore: SpyPlaybackCore())
        let onTimelineID = UUID()
        let libraryOnlyID = UUID()
        vm.records = [
            makeRecordWithCopilot(id: onTimelineID, copilot: dummyCopilot()),
            makeRecordWithCopilot(id: libraryOnlyID, copilot: nil),
        ]
        vm.timelineSegments = [
            TimelineSegment(id: UUID(), sourceVideoID: onTimelineID, range: TimeRange(startSeconds: 0, endSeconds: 5), text: "A", subtitles: []),
        ]
        XCTAssertTrue(vm.allTimelineClipsAnalyzed,
                      "A library-only unanalyzed record must not block the re-cut confirm")
    }
}
